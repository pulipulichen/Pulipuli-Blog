...
puliHandleComments = function puliHandleComments() {
  var pHC = this;
  pHC.title = "";
  ...
  pHC.handleComments = function (json) {
    var title = '';
    if (pHC.title !== '') title = '<h2>' + pHC.title + '</h2>';
    ...
    for (var i = 0, post; post = sortentry[i]; i++) {
      ...
      if (titlelen !== "" && title.length > titlelen) title = title.substr(0, titlelen) + "...";
      var link = "#";
  ...
  pHC.load = function (nodeID) {
    jQuery("#" + nodeID).html('<div id="' + pHC.divID + '"><h2>' + pHC.loading + '</h2></div>'); //console.log('a')

    var url = "/feeds/comments/full?alt=json-in-script&callback=?";
 ...
 * 會先執行initRecent
 */
var initRecent = function initRecent() {
...
var _load_random_posts = function _load_random_posts() {
  ...
  */
  ...
  var randomposts = function randomposts(json) {
    total_randomposts = json.feed.openSearch$totalResults.$t; //console.log({'total_randomposts': total_randomposts})
  ...
  var get_random = function get_random() {
    var ranNum = 1 + Math.round(Math.random() * (total_randomposts - 1));
    return ranNum;
  };

  var random_posts = function random_posts(json) {
    //for (var i = 0; i < randomposts_number; i++) {
    var entry = json.feed.entry[0]; //console.log(entry)
    ...
    ;
    var _li = ''; //document.write('<li>');

    _li = _li + '<dd>';
    _li = _li + '<a href="' + randompostsurl + '" rel="nofollow" title="' + randomposts_snippet + '">'; //document.write('<a href="' + randompostsurl + '" rel="nofollow"><img alt="' + randompoststitle + '" src="' + randompoststhumb + '"/></a>');

    _li = _li + '<img alt="' + randompoststitle + '" src="' + randompoststhumb + '"/>'; //document.write('<div><a href="' + randompostsurl + '" rel="nofollow">' + randompoststitle + '</a></div>');

    _li = _li + randompoststitle;

    if (randomposts_details === true) {
      //document.write('<span><div  class="random-info">' + randomposts_date.substring(8, 10) + '.' + randomposts_date.substring(5, 7) + '.' + randomposts_date.substring(0, 4) + ' - ' + randomposts_commentsnum) + '</div></span>'
      _li = _li + '<span><div  class="random-info">(' + randomposts_date.substring(0, 4) + '-' + randomposts_date.substring(8, 10) + '-' + randomposts_date.substring(5, 7) + ') ' + randomposts_commentsnum + '</span>';
    }
  ...
  var url = "/feeds/posts/default?alt=json-in-script&max-results=0&callback=?";

  var callback = function callback(data) {
    randomposts(data); //console.log(data)

    getvalue(); //for (var i = 0; i < randomposts_number; i++) {
    ...
    var _loop = function _loop(_i) {
      ...
      if (_i < randomposts_current.length) {
        ...
        var _url = '/feeds/posts/default?alt=json-in-script&start-index=' + randomposts_current[_i] + '&max-results=1&callback=?'; //console.log(url)


        lscacheHelper.getJSON(_url, function (data) {
          //console.log(data)
          random_posts(data);
        });
        _i++; //console.log(_i)
...
puliHandlePost = function puliHandlePost() {
  var pHP = this;
  pHP.title = "";
  ...
  pHP.load = function (nodeID) {
    ...
    if (tagname.substr(0, 3) !== "/-/" && tagname !== "") {
      tagname = "/-/" + tagname;
    }

    var url = "/feeds/posts/summary" + tagname + "/?alt=json-in-script&callback=?";
...
jQuery.puliGuestBook({
  blogID: "16607461",
  //blog的ID
  postID: "113544406852218769",
  //post的ID
  url: "/2005/12/blogger_113544406852218769.html#comment-editor",
  //訂閱張貼意見的網址，或是文章ID:115667103250300740
  //css: "//pulipulichen.github.io/Pulipuli-Blog/lib-for-link/modules/puliGuestBook/puliGuestBook.css",    //CSS樣式表
  container: ".puliGuestBook",
  //顯示留言的元素
  listNumber: 20,
  //顯示留言數量。超過此數量時，會顯示「閱讀全部留言」的連結。
  adminName: 'Pulipuli Chen',
  //Blog主人的名字
  adminPhoto: '//1.bp.blogspot.com/_yr4MQB4zDus/SZ4Mb30N0aI/AAAAAAAAFUg/3OGhwhzBUOg/S45/',
  //部落格主人的照片
  anonymous: '匿名',
  //匿名者的名字
  readMore: '閱讀全部留言',
  ...
  reload: '重新讀取',
  //重新讀取連結的名稱
  addLink: "/2005/12/blogger_113544406852218769.html#comment-editor",
  //撰寫留言的按鈕連結
...
var content = __webpack_require__(/*! !../../../../node_modules/css-loader/dist/cjs.js!../../../../node_modules/postcss-loader/src!./puli-guest-book.css */ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/src/index.js!./lib-for-link/src/global-footer/puli-guest-book/puli-guest-book.css");
...
jQuery.puliGuestBook = function (config) {
  var getParam = function getParam(index, defaultValue) {
  ...
  var getBaseLink = function getBaseLink() {
    var host = location.href; // by Pudding Chen 20120609
    //因為字尾會被換成tw，所以不能這樣做了
  ...
  if (container.length === 0) {
    ...
    document.write('<div id="' + _id + '">' //+ 'Loading...'
    + '</div>');
  ...
  if (blogID === null && postID === null) {
    ...
    if (url.substring(0, 7) !== 'http://') {
      ...
      host = host.substring(0, _slash_pos + 1); //url = host + 'feeds/' + url + '/comments/full?alt=json-in-script&callback=handleGuestbookPulipuli';
    ...
    if (url.substring(url.length - needle.length, url.length) === needle) {
      url = url.substring(0, url.length - needle.length); //url = url + 'full?alt=json-in-script&callback=handleGuestbookPulipuli';

      url = url + 'full?alt=json-in-script&callback=?';
    }
  } else {
    var host = getBaseLink(); //url = host + 'feeds/' + postID + '/comments/full?alt=json-in-script&callback=handleGuestbookPulipuli';
  ...
  var adminName = getParam('adminName', '布丁布丁吃布丁'); //var adminPhoto = getParam('adminPhoto', 'http://1.bp.blogspot.com/_yr4MQB4zDus/SZ4Mb30N0aI/AAAAAAAAFUg/3OGhwhzBUOg/S45/');
  ...
  var handleGuestbookPulipuli = function handleGuestbookPulipuli(json) {
    ...
    var firstPost = null; //console.log('========要顯示post了============')
    ...
    for (var i = 0, post; post = sortentry[i]; i++) {
      ...
      var commentId = post.id.$t;
      commentId = commentId.slice(commentId.lastIndexOf('.post-') + 6, commentId.length); //console.log(commentId)

      var commentLink = addLink;
      ...
      if (authorname === adminName && adminPhoto !== null) {
        authorname = '<img src="' + adminPhoto + '" class="admin-photo" border="0" /> ' + authorname;
      }
      ...
      var m = timestamp.substr(5, 2);
      var d = timestamp.substr(8, 2);
      var layout_replace = layout.replace("%comment%", title_link).replace("%Y%", y).replace("%M%", m).replace("%D%", d).replace("%authorname%", authorname).replace("%COMMENT_LINK%", commentLink);
      var odd = 0;
    ...
    var rss_link = '';
    var reload_cmd = ''; //if (firstPost != null)

    if (true) {
      var baseLink = null;

      if (blogID === null && postID === null && firstPost !== null) {
        link = firstPost.link[0].href; //http://www.blogger.com/feeds/16607461/1187506547871300947/comments/default/5372166020783852457
      ...
      $('#sidebar .guestbook a.icon.reload').click(function () {
        //jQuery.getScript(baseLink + "feeds/" + postID + "/comments/full?alt=json-in-script&callback=handleGuestbookPulipuli")
        var url = baseLink + "feeds/" + postID + "/comments/full?alt=json-in-script&callback=?";
    ...
    if (post = sortentry[i]) {
      link = add_link;
      temp += '<li class="guest-book-li readmore"><span class="item-title">' + '<a href="' + link + '#comments" target="guestbook_write">' + readMore + '</a>' + '</span></li>';
    }

    temp += "</ul>";

    if (disableBottomButtons === false) {
      temp += '<div class="guestbook-write">' //+ ' <a href="'+rss_link+'" style="float:right;margin-right:1em;font-size: 1.5em;line-height: 1em;">'
      //+ '<img src="http://3.bp.blogspot.com/_yr4MQB4zDus/Ru35yvgloDI/AAAAAAAABOQ/bbtw-pQhpOk/s200/rss.gif" border="0" />'
      //+ '<i class="fa fa-rss-square"></i>'
      //+ '</a>'
      + ' <a href="' + add_link + '" class="write" target="guestbook_write">' + write + '</a>' + ' <a class="write" onclick="' + reload_cmd + '">' + reload + '</a>' + '</div>';
...
/***/ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/src/index.js!./lib-for-link/src/global-footer/puli-guest-book/puli-guest-book.css":
/*!****************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/src!./lib-for-link/src/global-footer/puli-guest-book/puli-guest-book.css ***!
  \****************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
