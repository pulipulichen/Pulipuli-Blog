...
ArchiveListUtils = {
  openListYear: function openListYear(callback) {
    console.log('openListYear'); //$("#BlogArchive1_ArchiveList > ul.hierarchy > li.archivedate:not(.expanded) a.toggle:eq(0)").trigger("click")

    var li = $("#BlogArchive1_ArchiveList > ul.hierarchy > li.archivedate:not(.expanded)");
    var zippy = $("#BlogArchive1_ArchiveList > ul.hierarchy > li.archivedate:not(.expanded) > a.toggle > .zippy:not(.toggle-open)");
  ...
  openListMonth: function openListMonth(callback) {
    ...
    //console.log(111)
    this.openListYear(function () {
      //console.log(document.querySelectorAll("ul > li > ul > li.archivedate.collapsed > a.toggle").length)
      // 請溫柔地開它
      var aList = document.querySelectorAll("#BlogArchive1_ArchiveList > ul > li > ul > li.archivedate.collapsed > a.toggle");
  ...
  displayConfiguration: function displayConfiguration() {
    console.log('displayConfiguration');
    var cacheKey = 'archivesCheckboxStatus';
    this.openListYear(function () {
      // -------------------------------
      var checkboxChange = function checkboxChange() {
        // 要先確認自己是位於那個階層
        var folderType = $(this).attr('data-folder-type'); //console.log(folderType)
        ...
        if (folderType === 'year') {
          ...
          */
          var checkboxList = $(this).parent().find('ul > li > .download-checkbox'); //console.log(checkboxList.length)
          //console.log(checked)

          checkboxList.prop('checked', this.checked);
        } else {
          // 檢查上面一層底下的所有狀態，有跟自己不一樣的就移除上層的打勾
          var yearLi = $(this).parent().parent().parent(); //console.log(yearLi.prop('className'))

          var yearCheckbox = yearLi.children('.download-checkbox');
          var yearChecked = true;
          yearLi.find('ul.hierarchy > li > .download-checkbox').each(function (i, input) {
        ...
        var data = {};
        $('#BlogArchive1_ArchiveList input.download-checkbox').each(function (i, input) {
      ...
      $('#BlogArchive1_ArchiveList a.toggle').each(function (i, a) {
        // 查看一下現在的狀態，然後加上年月好嗎
        var folderType = 'month';
        ...
        if (folderType === 'year') {
          ...
          var link = $(a).parent().children('a.post-count-link').attr('href');
          var linkParts = link.split('/');
          value = 'd_' + linkParts[linkParts.length - 3] + '_' + linkParts[linkParts.length - 2];
        }

        var checkbox = $('<input type="checkbox" checked="checked" class="download-checkbox" />').attr('id', value).attr('data-folder-type', folderType).change(checkboxChange);
        $(a).before(checkbox).parent().addClass('init-download-checkbox');
      }); // --------------------------------
      // 讀取狀態
  ...
  // ------------------------------------------------------------------
  loopOpenItem: function loopOpenItem(linkList, callback) {
    var loop = function loop(i) {
      if (i < linkList.length) {
        var openRetryCount = 3;

        var openWindow = function openWindow() {
          var win = window.open(linkList[i]); // 這邊要等待它關閉

          var waitCount = 60 * 5;

          var checkWinClosed = function checkWinClosed() {
  ...
  startDownload: function startDownload(callback) {
    var _this = this;

    //console.log('startDownload')
    var checkedList = $('#BlogArchive1_ArchiveList input.download-checkbox[data-folder-type="month"]:checked');

    var loopChecklist = function loopChecklist(i) {
      if (i < checkedList.length) {
        var li = checkedList.eq(i).parent();
        ...
        if (li.hasClass('collapsed')) {
          ...
          // 已經點開了
          var linkList = [];
          li.find('ul.posts > li > a').each(function (i, ele) {
            linkList.push(ele.href + '?downloadArticle=true');
          }); // 那我們逐一點開吧

          _this.loopOpenItem(linkList, function () {
...
$(function () {
  var pageTool = $('#ArchiveList > .page-tool');
  pageTool.find('.open-list.year').click(function () {
    ArchiveListUtils.openListYear();
  });
  pageTool.find('.open-list.month').click(function () {
    ArchiveListUtils.openListMonth();
  });
  pageTool.find('.setup-checkbox').click(function () {
    ArchiveListUtils.displayConfiguration();
  }).click();
  pageTool.find('.start-download').click(function () {
...
var content = __webpack_require__(/*! !../../../../node_modules/css-loader/dist/cjs.js!../../../../node_modules/postcss-loader/src!../../../../node_modules/less-loader/dist/cjs.js!./style-page.less */ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/src/index.js!./node_modules/less-loader/dist/cjs.js!./lib-for-link/src/page/style/style-page.less");
...
/***/ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/src/index.js!./node_modules/less-loader/dist/cjs.js!./lib-for-link/src/page/style/style-page.less":
/*!********************************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/src!./node_modules/less-loader/dist/cjs.js!./lib-for-link/src/page/style/style-page.less ***!
  \********************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
